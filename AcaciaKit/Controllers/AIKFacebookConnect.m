...
-(void)requestPublishPermissions{
    if (![self canPublish]) {
        [self.facebookSession requestNewPublishPermissions:@[@"publish_actions"] defaultAudience:FBSessionDefaultAudienceFriends completionHandler:^(FBSession *session, NSError *error) {
            if (!error) {
                [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadName:KWFacebookAccountChangedNotification object:self userInfo:@{@"publish": @"true"}];
            }else{
                [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadName:KWFacebookAccountChangedNotification object:self userInfo:@{@"publish": @"false"}];
...
-(FBSession *)facebookSession{
    if (!_facebookSession) {
        ...
        _facebookSession = [[FBSession alloc] initWithAppID:kFacebookAppID permissions:permissions defaultAudience:FBSessionDefaultAudienceEveryone urlSchemeSuffix:nil tokenCacheStrategy:nil];
